...
static const unsigned RShift[64] = {
...
static const uint64_t RMagics[64] = {
...
static const unsigned BShift[64] = {
...
static const uint64_t BMagics[64] = {
...
static inline uint64_t king_attacks(int sq) {
  return KingAttacks[sq];
}

static inline uint64_t calc_king_attacks(uint64_t kings)
{
...
static inline uint64_t calc_knight_attacks(uint64_t knights)
{
  uint64_t h1 = ((knights >> 1) & 0x7F7F7F7F7F7F7F7FULL)
    | ((knights << 1) & 0xFEFEFEFEFEFEFEFEULL);
  uint64_t h2 = ((knights >> 2) & 0x3F3F3F3F3F3F3F3FULL)
    | ((knights << 2) & 0xFCFCFCFCFCFCFCFCULL);
  return (h1 << 16) | (h1 >> 16) | (h2 << 8) | (h2 >> 8);
}

static inline uint64_t pawn_attacks(unsigned cl, int sq) {
  return PawnAttacks[cl][sq];
}

static inline uint64_t calc_pawn_attacks(unsigned Cl, uint64_t pawns)
{
  if (Cl == 0) {  // White
...
static inline uint64_t rank_attacks(int sq, uint64_t occ)
{
...
static inline uint64_t file_attacks(int sq, uint64_t occ)
{
  static const uint64_t OuterSquares[64] = {
...
static inline uint64_t calc_rook_attacks(int sq, uint64_t occ)
{
  return file_attacks(sq, occ) | rank_attacks(sq, occ);
}

// Use Hyperbola Quintessence for bishop attack generation
static inline uint64_t diag_attacks(int sq, uint64_t occ)
{
  occ &= DiagMaskEx[sq];
  return ((occ - SquareMask[sq]) ^ __builtin_bswap64(__builtin_bswap64(occ) - SquareMask[sq ^ 56]))
    & DiagMaskEx[sq];
}

static inline uint64_t adiag_attacks(int sq, uint64_t occ)
{
  occ &= ADiagMaskEx[sq];
  return ((occ - SquareMask[sq]) ^ __builtin_bswap64(__builtin_bswap64(occ) - SquareMask[sq ^ 56]))
    & ADiagMaskEx[sq];
...
static inline uint64_t calc_bishop_attacks(int sq, uint64_t occ)
{
...
static inline uint64_t next_subset(const uint64_t Set, const uint64_t subset)
{
  return (subset - Set) & Set;
}

extern void m42_init();
extern void init_piece(bool rook, int sq);
#endif
